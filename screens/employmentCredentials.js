...
export async function renderEmploymentCredentials(root) {
  root.innerHTML = `
    <section class="page">
      <div class="hero">
        <h1>Wallet</h1>
  ...
  function renderGrid() {
    ...
    for (const dwaText of imported) {
      ...
      wrap.className = "credFlipCard";
      wrap.style.setProperty("--accent", color);

      wrap.innerHTML = `
        <div class="credFlipInner">
          <!-- FRONT -->
          <div class="credFace credFront">
            <div class="credFrontTop">
              <div class="credLogo">${escapeHtml(initialsForText(dwaText))}</div>
            </div>

            <div style="margin-bottom: 20px;" class="credTitle" title="${escapeHtml(dwaText)}">
              ${escapeHtml(dwaText)}
            </div>

            <div class="credStrip"></div>

            <div class="credFrontBottom">
          ...
          <!-- BACK -->
          <div class="credFace credBack">
            <div class="credBackLeft">
              <div class="credBackTitle">Credential</div>
              <div class="credBackSub">Related occupations</div>
              <hr class="credHr" />
              <div class="credBackMeta">
                <div><b>${jobCount}</b> related job${jobCount === 1 ? "" : "s"}</div>
                <div class="credMini">Click to view details.</div>
              </div>
            </div>

            <div class="credBackRight">
              <button class="credBtn" type="button" data-action="inspect">Inspect</button>
              <button class="credBtn ghost" type="button" data-action="remove">Remove</button>
            </div>

            <div class="credBackRail"></div>
          </div>
        </div>
      `;

      // Button actions
      wrap.querySelector('[data-action="inspect"]').addEventListener("click", (e) => {
        e.stopPropagation();
      ...
      // Clicking the card (not buttons) still inspects
